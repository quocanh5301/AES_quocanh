...
//        }
    ...
    public byte multiply(int a, int b) {
        int result = 0;
        for (int i = 0; i < 8; i++) {
            if ((b & 1) != 0) {
                result ^= a;
...
//            throw new IllegalArgumentException("Empty input, return an empty byte array");
                return;// Empty input, return an empty byte array
            }

            int paddingLength = paddedPlaintext[paddedPlaintext.length - 1] & 0xFF;  // Get the last byte as an integer

            if (paddingLength >= paddedPlaintext.length) {
//            throw new IllegalArgumentException("Invalid padding, return an empty byte array");  // Invalid padding, return an empty byte array
                return;
            }

            int originalLength = paddedPlaintext.length - paddingLength;
            System.arraycopy(paddedPlaintext, 0, plaintext, 0, originalLength);
        } else {
